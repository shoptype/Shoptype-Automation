Feature: Adult Content Filter Set
  ...
  Scenario: Coseller Sign Up
    ...
    When User chooses "Coseller" as a profile
    Then User should be signed in as "Coseller"

	@api
	Scenario: Import product from shopify with adult content filters
		...
		And Only adult content filter should be updated
	...
	Scenario: Search for the vendor from coseller account
		...
		Then Coseller should be able to see the list of products with "true:false,true" restriction value
